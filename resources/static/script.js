...
const transactionContainer = document.querySelector(".transaction-list__all-rows");
  ...
  // postTransaction.addEventListener("submit", function () {
  //   const formData = new FormData(form);
  //   const data = Object.fromEntries(formData);
  //   console.log(data);
  //
  //
  //   fetch("http://localhost:8080/api/saveTransaction", {
  //     method: "POST",
  //     headers: { "Content-Type": "application/x-www-form-urlencoded" },
  //     body: JSON.stringify(data),
...
allTransactions.then(res=>res.json()).then(result=> {
  ...
  expenseContainer.textContent = `$${totalExpense}`
    ...
    const totalBalance = balanceContainer.textContent =`$${incomeValue -expenseValue}`
...
async function loadCategoryLimits() {
  try {
    ...
    for (const result of data) {
      if (result.categoryName && result.totalAmtSpentThisMonth && result.percentage != null) {
        ...
        document
            .querySelector(".category-limit__all-categories")
        ...
        document.querySelectorAll(".category-limit__progress-bar").forEach(progressBar=>{
          ...
          if(getPercentage>=100){
            ...
            progressBar.style.backgroundColor = "rgb(246 135 147)";
            progressBar.style
          ...
          if(getPercentage<100){
            ...
            progressBar.style.backgroundColor = "#eaf8b0";
...
async function renderPieChart() {
  const data = await categorySpendingChart();
  console.log(data);
  const labels = data.map(row => row.categoryName);
  const amounts = data.map(row => row.totalSpending);
  const maxCategory = data.reduce((prev, curr) => prev.totalSpending > curr.totalSpending ? prev : curr);
  console.log(maxCategory)
  const canvasBackgroundText={
    id:'canvasBackgroundText',
    beforeDraw(chart, args, opts){
      const {
        ctx,width,height} = chart;
      ...
      ctx.fillStyle = '#555';
      ctx.fillText(opts.label, width / 2, height / 2 + 15);
    ...
    options:{
      ...
      plugins: {
        ...
        canvasBackgroundText: {
          amount: `$${maxCategory.totalSpending}`,
          label: `spent on ${maxCategory.categoryName}`
        }
